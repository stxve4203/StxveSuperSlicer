...
{
    ScalableButton* btn = new ScalableButton(this, wxID_ANY, bmp_name, text, wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT | wxBORDER_SIMPLE | (bmp_name.empty() ? 0 : wxBU_LEFT), false, bmp_size);
    
    // Set custom background color for the button
    wxColour custom_bg_color = *wxBLUE; // You can change this to any color you like
    btn->SetBackgroundColour(custom_bg_color);
